...
export class AuthService  {
    ...
    async signUp (fullName: string, email: string, password: string, role: string) {
        ...
        //     role
        // }
        // const addUser2 = await this.userRepository.createUser(userData)
    ...
    async verifySignUpOtp  (email: string, otp: string) {
        ...
        if (otp === getOtp?.otp) {
            ...
            if (checkUser.role === 'mentee') {
                return { success: true, message: 'OTP verified successfully', role: checkUser.role, accessToken, refreshToken,user:checkUser}
            } else {
                await sendMentorData(checkUser)
                return { success: true, message: 'OTP verified successfully', role: checkUser.role , user:checkUser }
            }
        } else {
            return { success: false, message: 'Invalid otp' }
        }
