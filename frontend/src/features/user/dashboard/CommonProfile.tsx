...
const CommonProfile = ({ userData }) => {
    ...
    const { register, handleSubmit, formState: { errors },reset } = useForm<commonType>({
    ...
    useEffect(()=>{
        const dataSetting = async()=>{
            if (!selectedFile) {
                if (userData?.profile && userData.profile !== 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_640.png') {
